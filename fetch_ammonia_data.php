...
$query = "SELECT ammonia_level, last_saved FROM sensor_data ORDER BY last_saved DESC LIMIT 5";
$stmt = $connpdo->prepare($query);
$stmt->execute();
$ammonia_data = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch the user's safe pH range
$safe_query = "SELECT AMMONIA_MIN, AMMONIA_MAX FROM safe_range WHERE USER_ID = :user_id";

...
$query = "SELECT temperature, last_saved FROM sensor_data ORDER BY last_saved DESC LIMIT 5";
$stmt = $connpdo->prepare($query);
$stmt->execute();
$temp_data = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch the user's safe pH range
$safe_query = "SELECT TEMP_MIN, TEMP_MAX FROM safe_range WHERE USER_ID = :user_id";

...
$query = "SELECT do_level, last_saved FROM sensor_data ORDER BY last_saved DESC LIMIT 5";
$stmt = $connpdo->prepare($query);
$stmt->execute();
$do_data = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch the user's safe pH range
$safe_query = "SELECT DO_MIN FROM safe_range WHERE USER_ID = :user_id";
